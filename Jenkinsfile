...
node(node_label) {
    ...
    def image_tag = "${docker_repo}/cdcagg-client:${env.BRANCH_NAME.toLowerCase().replaceAll('[^a-z0-9\\.\\_\\-]', '-')}-${env.BUILD_NUMBER}"
    ...
    try {
        ...
        if (env.BRANCH_NAME == 'main') {
            stage('Push Docker image') {
                ...
                sh "gcloud container images add-tag ${image_tag} ${docker_repo}/cdcagg-client:${env.BRANCH_NAME}-latest"
            }
        }
    } catch (err) {
